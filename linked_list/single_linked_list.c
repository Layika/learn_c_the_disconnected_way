#include <stdio.h>
#include <stdlib.h>
#define ASSERT(x) if (!(x)) printf("Assertion error: " #x " in function %s, line %d\n", __func__, __LINE__);	
...
void list_append(node** list, data_t new_element){
...
void list_insert(node** list, data_t new_element, data_t position){
  node* new_node = (node*)malloc(sizeof(node));
  CHECK_ALLOC(new_node);
...
void list_reverse(node** list){
  ...
  void test_list_remove_too_big_position(void){
    node* list = list_create(0);
    list_append(&list, 1); 
...
int main(void){
  ...
  test_list_remove();
  test_list_reverse(); 
  test_list_remove_too_big_position();
